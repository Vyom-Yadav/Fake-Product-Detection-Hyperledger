...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o orderer1.orderer.supplychain.com:7050 --ordererTLSHostnameOverride orderer1.orderer.supplychain.com -c "$CHANNEL" --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
